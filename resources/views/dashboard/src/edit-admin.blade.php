...
<html lang="ar" dir="rtl">
  <head>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
      <title>Vela: المشرفين تعديل</title>
    <!-- Google Fonts Cairo -->
  ...
  <body>
    <div class="table-container section-style">
        <div style="border-bottom: 1px solid #ddd; margin-bottom: 40px; padding-bottom: 10px">
            <h4>تعديل مشرف <span class="text-vela">{{$user->full_name}}</span></h4>
        </div>
        <form
            class="row"
            style="row-gap: 20px"
            method="post"
            action="{{route('admins.update',['user'=>$user->id])}}"
            ...
            <div class="col-md-6">
                <label for="company_name" class="form-label">اسم الشركة <span style="color:red;">*</span></label>
                <input value="{{$user->company_name}}" type="text" name="company_name" class="form-control @error('company_name') is-invalid @enderror" id="company_name" />
                @error('company_name')
                <span class="text-danger">{{$message}}</span>
                @enderror
            </div>
